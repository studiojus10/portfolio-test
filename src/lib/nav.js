...
export function renderNav() {
  ...
  var eu       = p.startsWith('photography-europe');
  var projects = p.startsWith('projects');
  var sketches   = p === 'art-sketches.html';
  var composers  = p === 'art-composers.html';
  ...
  function desktopLink(href, label, cjk, active) {
    return lnk(href,
      ...
      '<span>' + label + '</span>' +
        '<span class="font-cjk text-[13px] text-primary">' + cjk + '</span>'
    );
  }
  ...
  function mobileTopLink(href, label, cjk, active) {
    return lnk(href,
      'flex justify-between items-center border-b border-on-background pb-4 hover:text-primary transition-colors' +
        (active ? ' text-primary' : ' text-on-background'),
  ...
  function mobileSubItem(href, label, cjk, active) {
    return lnk(href,
      ...
      '<span class="text-[12px] tracking-widest italic">' + label + '</span>' +
  ...
  // ── Build HTML ───────────────────────────────────────────────────
  var html =
    '<nav class="bg-surface-container-lowest font-label-caps text-label-caps w-full top-0 sticky border-b border-on-background flex justify-between items-center px-4 md:px-8 py-4 z-[99999]">' +
    ...
    lnk('/index.html',
      ...
      'STUDIO JUS10<span class="font-cjk text-lg font-light text-primary hidden sm:inline-block ml-2">工作室</span>'
    ) +
    ...
    lnk('/photography.html',
      ...
      '<span>Photography</span>' +
        '<span class="font-cjk text-[13px] text-primary">摄影</span>'
    ) +
    '<div class="absolute top-full left-1/2 -translate-x-1/2 w-52 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity duration-200 z-[99999]">' +
    '<div class="h-3"></div>' +
    ...
    dropItem('/photography-film.html',       'Film',       '胶片',     film, false) +
    dropItem('/photography-europe.html',     'Europe',     '欧洲',     eu,   true) +
    '</div></div></div>' +
    ...
    lnk('/art.html',
      (art ? 'text-primary' : 'text-on-surface hover:text-primary') +
        ' transition-colors duration-300 cursor-pointer flex flex-col items-center',
      '<span>Art</span>' +
        '<span class="font-cjk text-[13px] text-primary">艺术</span>'
    ) +
    '<div class="absolute top-full w-44 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity duration-200 z-[99999]" style="right:0;">' +
    '<div class="h-3"></div>' +
    '<div class="bg-surface-container-lowest border border-on-background shadow-lg">' +
    dropItem('/art-sketches.html',   'Sketches',   '素描',   sketches,  false) +
    dropItem('/art-composers.html',  'Composers',  '作曲家', composers, true) +
    '</div></div></div>' +

    desktopLink('/projects.html', 'Projects', '项目', projects) +

    desktopLink('/about/about.html', 'About', '关于', about) +
    '</div>' +
    ...
    lnk('/contact.html',
      ...
      '<span>Contact</span><span class="font-cjk text-xs text-primary">联系</span>'
    ) +
    ...
    // Mobile menu
    '<div id="mobile-menu" class="fixed inset-y-0 right-0 z-[99998] w-72 bg-surface-container-lowest border-l border-on-background transform translate-x-full transition-transform duration-300 md:hidden flex flex-col pt-20 px-6 gap-6 font-label-caps text-label-caps tracking-widest overflow-y-auto">' +
    '<div>' +
    lnk('/photography.html',
      'flex justify-between items-center pb-3 ' + (photo ? 'text-primary' : 'text-on-background hover:text-primary transition-colors'),
    ...
    mobileSubItem('/photography-europe.html',     'Europe',     '欧洲',     eu) +
    '</div></div>' +
    mobileTopLink('/video.html', 'Video', '视频', video) +
    '<div>' +
    lnk('/art.html',
      'flex justify-between items-center pb-3 ' + (art ? 'text-primary' : 'text-on-background hover:text-primary transition-colors'),
      '<span>Art</span><span class="font-cjk text-primary text-sm">艺术</span>'
    ) +
    '<div class="pl-3 flex flex-col gap-1 border-b border-on-background pb-4">' +
    mobileSubItem('/art-sketches.html',  'Sketches',  '素描',   sketches) +
    mobileSubItem('/art-composers.html', 'Composers', '作曲家', composers) +
    '</div></div>' +
    mobileTopLink('/projects.html', 'Projects', '项目', projects) +
    mobileTopLink('/about/about.html', 'About', '关于', about) +
    ...
    '<div id="mobile-menu-overlay" class="fixed inset-0 bg-black/50 z-[99997] opacity-0 pointer-events-none transition-opacity duration-300 md:hidden"></div>';
